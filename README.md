...
任何模型权重 —— 请自行放到 [`models/`](models/) 下（见
[`models/README.md`](models/README.md)）。模型转换（ONNX → `.hbm`、PTQ 校准）
...
| DeepLabV3+、SigLIP | 语义分割、图像嵌入 | 来自 D-Robotics `rdk_model_zoo`（请核对其条款） |
| SCRFD、ArcFace（insightface） | 人脸检测 / 识别 | **MIT**（insightface 代码）—— 但**预训练权重是学术非商用**。商用形态请换 **YuNet**（OpenCV Zoo，纯 CNN，int8 安全）+ 另择识别骨干 |
| ViTPose（easy_ViTPose 导出） | 全身姿态 133 点 | **Apache-2.0** |
| XFeat | 稀疏特征点 | **Apache-2.0** |
| SPAN | 超分 | **Apache-2.0** |
| Real-ESRGAN（realesr-general-x4v3） | 超分 | **BSD-3-Clause** |
| YOLOP | 全景驾驶感知 | **MIT** |
| PIDNet | 实时语义分割 | **MIT** |
